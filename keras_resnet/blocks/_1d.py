...
def basic_1d(filters, stage=0, block=0, kernel_size=3, strides=None, **kwargs):
    """
    ...
    def f(x):
        ...
        y = keras.layers.Conv1D(filters, kernel_size, strides=strides, name="res{}{}_branch2a".format(stage_char, block_char), **convolution_kwargs)(y)
        ...
        if block == 0:
            shortcut = keras.layers.Conv1D(filters, (1, 1), strides=strides, name="res{}{}_branch1".format(stage_char, block_char), **convolution_kwargs)(x)
...
def bottleneck_1d(filters, stage=0, block=0, kernel_size=3, strides=None, **kwargs):
    """
    ...
    if strides is None:
        strides = 1 if block != 0 or stage == 0 else 2
    ...
    def f(x):
        y = keras.layers.Conv1D(filters, (1, 1), strides=strides, name="res{}{}_branch2a".format(stage_char, block_char), **convolution_kwargs)(x)
        y = keras.layers.BatchNormalization(axis=axis, name="bn{}{}_branch2a".format(stage_char, block_char), **batch_normalization_kwargs)(y)
        y = keras.layers.Activation("relu", name="res{}{}_branch2a_relu".format(stage_char, block_char))(y)
        ...
        if block == 0:
            shortcut = keras.layers.Conv1D(filters * 4, (1, 1), strides=strides, name="res{}{}_branch1".format(stage_char, block_char), **convolution_kwargs)(x)
            shortcut = keras.layers.BatchNormalization(axis=axis, name="bn{}{}_branch1".format(stage_char, block_char), **batch_normalization_kwargs)(shortcut)
        else:
            shortcut = x
